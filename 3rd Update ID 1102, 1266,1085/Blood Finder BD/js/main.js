...
class BloodDonationApp {
  ...
  updateStats() {
    const donorCount = this.users.filter(
  ...
  findEmergencyDonors(bloodGroup, location) {
    const compatibleDonors = this.users.filter(
      (user) =>
        user.userType === "donor" &&
    ...
    if (compatibleDonors.length > 0) {
      alert(
        `Found ${compatibleDonors.length} eligible donors in ${location} with ${bloodGroup} blood group.`
      );
  ...
  findCompatibleDonors(bloodGroup, location) {
    ...
    const compatibleDonors = this.users.filter((user) => {
      ...
      return (
        compatibility[user.bloodGroup].includes(bloodGroup) &&
        user.location.district.toLowerCase().includes(location.toLowerCase()) &&
        this.isEligibleToDonate(user)
      );
    });
    ...
    if (compatibleDonors.length > 0) {
      alert(
        `Found ${compatibleDonors.length} compatible donors in ${location}.`
      );
    } else {
  ...
  loadDonorDashboard() {
    ...
    const userDonations = this.donations.filter(
      (d) => d.donorId === this.currentUser.id
    );
    ...
    dashboard.innerHTML = `
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="card ${
                      this.isEligibleToDonate(this.currentUser)
                        ? "bg-success"
                        : "bg-warning"
                    } text-white">
                        <div class="card-body">
                            ...
                            ${
                              !this.isEligibleToDonate(this.currentUser)
                                ? `<small>Next eligible: ${nextEligibleDate}</small>`
                                : ""
                            }
                        </div>
                    </div>
...
function handleRequestBloodClick(event) {
  ...
  if (currentUser && currentUser !== "null") {
    ...
    if (window.location.pathname.includes("dashboard.html")) {
      // Already on dashboard, show request modal directly
      ...
      // On homepage, redirect to dashboard
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body text-center">
                        ...
                        <h6>To request blood, you need to be logged in</h6>
                        <p class="text-muted">Do you have an account?</p>
                    </div>
                    <div class="modal-footer d-flex justify-content-center">
                        <button type="button" class="btn btn-outline-danger me-2" onclick="goToRegister()">
                            No, Create Account
                        </button>
                        <button type="button" class="btn btn-danger" onclick="goToLogin()">
                            Yes, Login
                        </button>
                    </div>
                </div>
...
// Bind Request Blood button
document.addEventListener("DOMContentLoaded", function () {
  const requestBloodBtn = document.getElementById("requestBloodBtn");
  if (requestBloodBtn) {
    requestBloodBtn.addEventListener("click", handleRequestBloodClick);
  }
});
