...
class CRUDController {
  ...
  updateBloodRequest(id, updateData) {
    const requestIndex = this.bloodRequests.findIndex(
    ...
    return this.users.filter((user) => {
      ...
      if (
        ...
        !user.location.district
          .toLowerCase()
          .includes(criteria.district.toLowerCase())
      ) {
        matches = false;
      }
      ...
      if (
        ...
        !user.location.upazila
          .toLowerCase()
          .includes(criteria.upazila.toLowerCase())
      ) {
        matches = false;
      }
  ...
  searchHospitals(criteria) {
    return this.hospitals.filter((hospital) => {
      ...
      if (
        criteria.type &&
        hospital.type.toLowerCase() !== criteria.type.toLowerCase()
      ) {
        matches = false;
      }
  ...
  getDonationStats() {
    ...
    const fulfilledRequests = this.bloodRequests.filter(
  ...
  getBloodGroupStats() {
    ...
    bloodGroups.forEach((group) => {
      stats[group] = {
        donors: this.users.filter(
          (u) => u.userType === "donor" && u.bloodGroup === group
        ).length,
        requests: this.bloodRequests.filter((r) => r.bloodGroup === group)
          .length,
